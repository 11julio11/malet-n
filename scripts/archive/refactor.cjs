...
console.log("Refactored ArchitectureModal.jsx");

// Fix index.css
const cssPath = 'C:/Mis_Proyectos(github)/malet-n/src/index.css';
let css = fs.readFileSync(cssPath, 'utf8');

// Convert px fonts to rem
css = css.replace(/font-size:\s*(\d+)px/g, (match, p1) => {
    return `font-size: ${p1 / 16}rem`;
});

// Convert rigid widths to clamp or elastic
css = css.replace(/width:\s*(\d{3,})px/g, (match, p1) => {
    return `width: 90vw; max-width: ${p1}px`;
});
